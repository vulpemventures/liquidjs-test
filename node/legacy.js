...
async function main() {
  ...
  const alice = liquid.payments.p2pkh({ pubkey: keyPair.publicKey, network });
  const bob = liquid.payments.p2pkh({ pubkey: keyPair2.publicKey, network });
  ...
  await sleep(1500);
  // Get the whole previous tx as hex Buffer
